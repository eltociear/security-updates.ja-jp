...
| Windows XP 上の Internet Explorer 6 Service Pack 1 (64-Bit Edition) ![](../../images/Dn609963.mu_s(ja-JP,Security.10).gif)                                                                                                                                                                   | [![](../../images/Dn609963.dl_arrow(ja-JP,Security.10).jpg)](https://www.microsoft.com/download/details.aspx?familyid=c05103e8-4402-4d54-ba03-fbbc24142e4d&displaylang=ja) | -                                                                                                                                                                                                     |
...
Internet Explorer がドラッグ アンド ドロップを処理する方法に脆弱性があるため、攻撃者が[特権の昇格](https://www.microsoft.com/japan/security/glossary.mspx)をする可能性があります。 攻撃者がこの脆弱性を悪用するには、悪質な Web ページや電子メールを作成し、それを表示したユーザーのシステム上に、ファイルを保存する必要があります。攻撃者はこの脆弱性を悪用し、影響を受けるコンピュータを完全に制御する可能性があります。 しかし、これを行うには、ユーザー側での操作も必要となります。
...
DHTML イベントとは DHTML オブジェクト モデルにより提供される特別な動作です。 これらのイベントはスクリプトコードで使用され、Web サイトに動的なコンテンツを追加します。 DHTML イベントの詳細については、[MSDN](https://msdn2.microsoft.com/en-us/library/ms533051.aspx) (英語情報） をご覧ください。
...
いいえ。Windows 98、Windows 98 Second Edition、および Windows Millennium Edition には影響を受けるコンポーネントが含まれますが、この脆弱性の深刻度は「緊急」ではありません。深刻度評価システムに関する詳細情報は、次のマイクロソフト [Web サイト](https://technet.microsoft.com/security/bulletin/rating)をご覧下さい。
...
はい。このセキュリティ更新プログラムは、現在悪用される可能性のある脆弱性を解決します。 解決される脆弱性は Common Vulnerability and Exposure の番号 [CAN-2004-0839](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=can-2004-0839) に割り当てられています。
...
これは、なりすましの脆弱性です。Inteｒnet Explorer が現在表示している Web サイトのアドレスを表示するために使用するアドレス バーに影響を与えます。 この脆弱性により、Internet Explorer で表示されている Web ページとは異なる不正な URL がアドレス バーに表示される可能性があります。 たとえば、ユーザーがリンクをクリックする時にアドレスバーには https://www.tailspintoys.com と表示されるリンクを、攻撃者が作成する可能性があります。しかし、実際には Internet Explorer では https://www.wingtiptoys.com などといった別の Web サイトが表示されます。
...
2 バイト文字セット (DBCS) とは、最小単位を 1 バイト とする ８ ビットの文字セットの拡張版です。 DBCS の文字のコード値には、シングル バイトと 2 バイトが含まれます。 DBCS というと、Microsoft Windows の一部のアジア バージョンで使用される ANSI 文字セットがあります。 DBCS の詳細については、MSDN の[Double-byte Character Sets](https://msdn2.microsoft.com/en-us/library/ms776454.aspx) (英語情報） をご覧ください。

**DBCS** **ロケールを実行しているかどうかはどのように判別できますか？**

DBCS には、Microsoft Windows の一部のアジア バージョンで使用される ANSI 文字セットがあります。 通常これは、中国語、日本語、韓国語のみで使用されています。システム ロケールおよびその確認方法に関する情報は、こちらのマイクロソフト [Web サイト](https://www.microsoft.com/globaldev/reference/oslocversion.mspx) (英語情報）をご覧ください。
...
これは、Internet Explorer が、現在アクセスしている Web サイトを表示するために使用するアドレス バーに影響する脆弱性で、なりすましが実行される可能性があります。 この脆弱性により、Internet Explorer で表示されている Web ページとは異なる不正な URL を、アドレス バーに表示される可能性があります。 たとえば、攻撃者は、ユーザーが一度クリックすると、実際の Web サイトには、https://www.wingtiptoys.com のコンテンツが表示されるにもこ関わらず、アドレス バーには https://www.tailspintoys.com と表示されるリンクを作成する可能性があります。
...
いいえ。Windows 98、Windows 98 Second Edition、および Windows Millennium Edition には影響を受けるコンポーネントが含まれますが、この脆弱性の深刻度は「緊急」ではありません。深刻度評価システムに関する詳細情報は、こちらのマイクロソフト [Web サイト](https://technet.microsoft.com/security/bulletin/ms04-004)をご覧下さい。
...
Internet Explorer が画像タグのスクリプトを処理する方法に[特権の昇格](https://www.microsoft.com/japan/security/glossary.mspx)の脆弱性が存在します。 攻撃者この脆弱性を悪用するには、悪質な Web ページや電子メールを作成し、それを表示したユーザーのシステム上に、ファイルを保存することが攻撃者にとっての必要条件となります。この脆弱性を悪用できた場合、そのコンピュータを完全に制御できる可能性があります。 しかし、これを行うには、ユーザー側での操作も必要となります。
...
-   攻撃者はこの脆弱性を悪用し、ユーザーと同じ特権を取得する可能性があります。システムに対する特権が少ないユーザーは、管理者特権で実行しているユーザーよりも危険にさらされる可能性が低減されます。
-   既定で、Outlook Express 6、Outlook 2002、および Outlook 2003 は、HTML 形式の電子メールを制限付きサイト ゾーンで開きます。 また、Outlook 98 および Outlook 2000 は、[Outlook 電子メール セキュリティ更新プログラム](https://www.microsoft.com/download/details.aspx?familyid=8abb391e-d223-4843-a0cb-00c6be66c55e&displaylang=ja)がインストールされている場合、HTML 形式の電子メール メッセージを制限付きサイト ゾーンで開きます。 また、Outlook Express 5.5 Service Pack 2 は、[MS04-018](https://technet.microsoft.com/security/bulletin/ms04-018) がインストールされている場合、HTML 形式の電子メール メッセージを制限付きサイト ゾーンで開きます。制限付きサイト ゾーンにより、この脆弱性を悪用する攻撃を受ける可能性が低減されます。

    次のすべての条件があてはまる場合、HTML 形式の電子メールによる攻撃の危険は著しく低減されます。

    -   マイクロソフト セキュリティ情報 [MS03-040](https://technet.microsoft.com/security/bulletin/ms03-040) またはそれ以降の Internet Explorer 用の累積的なセキュリティ更新プログラムに含まれる更新プログラムを適用している
    -   Outlook Express 5.5 Service Pack 2 以降を使用しており、マイクロソフト セキュリティ情報 [MS04-018](https://technet.microsoft.com/security/bulletin/ms04-018) 以降の Outlook Express 用の累積的なセキュリティ更新プログラムに含まれる更新プログラムを適用している
    -   Microsoft Outlook 電子メール セキュリティ アップデートがインストールされている Microsoft Outlook 98 および Outlook 2000 を使用している
...
はい。このセキュリティ更新プログラムは、現在悪用される可能性のある脆弱性を解決します。 解決される脆弱性は Common Vulnerability and Exposure の番号 [CAN-2004-0841](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=can-2004-0841) に割り当てられています。
...
[SSL (Secure Sockets Layer)](https://www.ietf.org/rfc/rfc2246.txt) とは、セキュリティを高めるために、Web セッションを暗号化するプロトコルです。 Internet Explorer のブラウザ ウィンドウの右下隅に黄色い鍵のアイコンが表示されていたら、現在のセッションが SSL で保護されていることを意味します。
...
**注** : これらのスイッチを 1 つのコマンド ラインに組み込むことができます。旧バージョンとの互換性のため、このセキュリティ更新プログラムは、セットアップ ユーティリティのより前のバージョンによって使用されるセットアップ スイッチもサポートしています。サポートされるインストール スイッチについての追加情報はマイクロソフト サポート技術情報 [262841](https://support.microsoft.com/kb/262841) をご覧ください。
...
この更新プログラムを SMS で展開する方法に関する情報は、サポート技術情報 [887437](https://support.microsoft.com/kb/887437) をご覧ください。
...
**注** : IE 5.5 SP2 は、現在 Windows Millennium Edition (ME) 上のみでサポートされています。 Internet Explorer 5.5 SP2 のサポート ライフサイクルに関する詳細情報は、[マイクロソフト ライフサイクル](https://support.microsoft.com/gp/lifeprodi)をご覧下さい。 Windows Me は、現在延長サポートの提供期間中です。 Windows Me のサポートに関する詳細は、次の[Web サイト](https://support.microsoft.com/gp/lifean1)をご覧ください。
...
サポートされているセットアップ スイッチに関する追加情報は、マイクロソフト サポート技術情報 [197147](https://support.microsoft.com/kb/197147) をご覧ください。
...
-   インストール エンジンの脆弱性 (CAN-2004-0216) を報告してくださった [KPMG](https://www.kpmg.co.uk/) UK 社の Greg Jones 氏 および [Next Generation Security Software Ltd](https://www.ngssoftware.com/) 社の Peter Winter-Smith 氏
-   SSL キャッシュの脆弱性 (CAN-2004-0845) を報告してくださった [ACROS Security](https://www.acrossecurity.com/) の Mitja Kolsek 氏
-   セキュリティ更新プログラムによって、 Hrtbeat.ocx に kill-bit が設定されていることによる現象について報告してくださった [Next Generation Security Software Ltd](https://www.ngssoftware.com/) 社の John Heasman 氏
-   2 バイト文字セットを使用するシステムにおけるアドレス バーのなりすましの脆弱性 (CAN-2004-0844) および、プラグイン ナビゲーシン アドレス バーを使用したなりすましの脆弱性 (CAN-2004-0843) を報告してくださった [Liu Die Yu](https://umbrella.name/)
...
-   US マイクロソフトセキュリティ情報（MS04-038）  
    [https://www.microsoft.com/technet/security/bulletin/ms04-038.mspx](https://technet.microsoft.com/security/bulletin/ms04-038)
-   サポート技術情報 （KB） 文書番号 ：834707  
    [\[MS04-038\] Internet Explorer 用の累積的なセキュリティ更新プログラム](https://support.microsoft.com/kb/834707)
