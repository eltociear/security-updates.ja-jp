---
TOCTitle: 'MS10-003'
Title: 'マイクロソフト セキュリティ情報 MS10-003 - 重要'
ms:assetid: 'ms10-003'
ms:contentKeyID: 61229174
ms:mtpsurl: 'https://technet.microsoft.com/ja-JP/library/ms10-003(v=Security.10)'
---
...
[](https://www.microsoft.com/japan/security/bulletins/ms10-003e.mspx)[![](../../images/Dn627160.SNS300x50(ja-JP,Security.10).jpg)](https://profile.microsoft.com/regsysprofilecenter/subscriptionwizard.aspx?wizid=cbdde499-aa75-4a75-9cb3-f48eac2e7c3f&lcid=1041)
...
既知の問題: マイクロソフト サポート技術情報 [978214](https://support.microsoft.com/kb/978214) には、このセキュリティ更新プログラムをインストールする際に起こる可能性のある既知の問題に関して説明されています。

US マイクロソフトセキュリティ情報: [https://www.microsoft.com/technet/security/bulletin/MS10-003.mspx](https://technet.microsoft.com/security/bulletin/ms10-003)
...
![](../../images/Dn627160.dl_arrow(ja-JP,Security.10).jpg)このマークをクリックして、PC/AT 互換機用または NEC PC-9800 シリーズ用の更新プログラムをダウンロードしてください。
...
<td style="border:1px solid black;">
[![](../../images/Dn627160.dl_arrow(ja-JP,Security.10).jpg)](https://www.microsoft.com/download/details.aspx?familyid=47553f45-fa10-40e5-8267-9d42ff560a62&displaylang=ja)
</td>
<td style="border:1px solid black;">
リモートでコードが実行される
</td>
...
<td style="border:1px solid black;">
[![](../../images/Dn627160.dl_arrow(ja-JP,Security.10).jpg)](https://www.microsoft.com/download/details.aspx?familyid=7c985595-00c5-44b8-81c3-59d9967220f8&displaylang=ja)
</td>
<td style="border:1px solid black;">
リモートでコードが実行される
</td>
...
MS10-004 もまた Microsoft Office 2004 for Mac に存在する脆弱性を説明しています。 MS10-004 はこのセキュリティ情報 (MS10-003) とどのような関係があるのですか?
Microsoft Office 2004 for Mac の累積的なサービス モデルの一部として、この Microsoft Office 2004 for Mac (KB979674) 用のセキュリティ更新プログラムも [MS10-004](https://technet.microsoft.com/security/bulletin/ms10-004) で説明している脆弱性を解決します。Microsoft Office 2004 for Mac をインストールしている場合、このセキュリティ更新プログラムをインストールする必要がありますが、インストールが必要なのは一回のみです。
...
次の深刻度の評価は脆弱性による影響が最大であった場合を前提としています。このセキュリティ情報のリリース後 30 日以内に、深刻度の評価およびセキュリティ上の影響に関連する脆弱性が悪用される可能性に関する情報は、[2 月 月のセキュリティ情報](https://technet.microsoft.com/security/bulletin/ms10-feb)の「Exploitability Index (悪用可能性指標)」のセクションをご覧ください。 詳細については、[Microsoft Exploitability Index (悪用可能性指標)](https://technet.microsoft.com/security/cc998259.aspx)をご覧ください。
...
<th style="border:1px solid black;" >
MSO.DLL のバッファー オーバーフロー - [CVE-2010-0243](https://cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2010-0243)
</th>
<th style="border:1px solid black;" >
総合的な深刻度
</th>
...
Microsoft Office が特別な細工がされた Office ファイルを処理する方法に、リモートでコードが実行される脆弱性が存在します。攻撃者がこの脆弱性を悪用した場合、影響を受けるコンピューターを完全に制御する可能性があります。その後、攻撃者はプログラムのインストール、データの表示、変更、削除、または完全なユーザー権限を持つ新たなアカウントを作成する可能性があります。コンピューターで、アカウントのユーザー権限を低く設定している場合、管理者ユーザー権限で実行しているユーザーよりもこの脆弱性の影響が少なくなると考えられます。

Common Vulnerabilities and Exposures のリストの標準のエントリとしてこの脆弱性をご覧になるためには、[CVE-2010-0243](https://cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2010-0243) をご覧ください。

#### 「MSO.DLL のバッファー オーバーフロー」の問題を緩和する要素 - CVE-2010-0243:
...
-   Web ベースの攻撃のシナリオで、攻撃者はこの脆弱性の悪用を意図した Office ファイルを含む Web サイトをホストする可能性があります。さらに、侵害された Web サイトおよびユーザーが提供したコンテンツまたは広告を受け入れる、またはホストしている Web サイトに特別に細工したコンテンツが含まれ、この脆弱性を悪用する可能性があります。しかし、すべての場合において、これらの Web サイトに強制的にユーザーを訪問させることはできません。 その代わり、通常、攻撃者は電子メール メッセージまたはインスタント メッセンジャーのメッセージ内のリンクをクリックさせて、ユーザーを攻撃者の Web サイトに誘導し、特別に細工した Office ファイルを表示させることが攻撃者にとっての必要条件となります。

#### 「MSO.DLL のバッファー オーバーフロー」の回避策 - CVE-2010-0243:

回避策は、設定または構成の変更を示しており、基本的な脆弱性を正すものではありませんが、更新プログラムを適用する前に既知の攻撃方法を阻止するのに役立ちます。マイクロソフトは、以下の回避策を検証し、この回避策が機能性を低下させるかどうかについて、説明の部分で述べています。

-   信頼できないソースからの Office ファイルを開かない

    信頼できない、または信頼できるソースから予期せず受け取った Microsoft Office ファイルを開かないでください。ユーザーが特別な細工がされたファイルを開いた場合、この脆弱性が悪用される可能性があります。

#### 「MSO.DLL のバッファー オーバーフロー」のよく寄せられる質問 - CVE-2010-0243:
...
ユーザーが影響を受けるバージョンのソフトウェアを使用して、特別に細工された Office ファイルを開いた場合、この脆弱性が悪用される可能性があります。

電子メールでの攻撃のシナリオでは、攻撃者は特別な細工がされた Office ファイルをユーザーに送信し、ユーザーにそのファイルを開くよう誘導することにより、この脆弱性を悪用する可能性があります。

Web ベースの攻撃のシナリオでは、この脆弱性の悪用を意図した Office ファイルが Web サイトに含まれる可能性があります。 攻撃者はユーザーを攻撃者の Web サイトに訪問させ、特別に細工した Office ファイルを表示させようとします。一般的には、ユーザーに電子メール メッセージまたはインスタント メッセンジャーのメッセージのリンクをクリックさせ、攻撃者の Web サイトへ誘導します。

主にどのようなコンピューターがこの脆弱性による危険にさらされますか?
主に、ワークステーションやターミナル サーバーなどの影響を受ける製品を使用しているシステムが、この脆弱性による危険にさらされます。管理者がユーザーにサーバーへのログオンおよびプログラムの実行を許可している場合、サーバーが影響を受ける可能性が高くなります。しかし、セキュリティ上の最善策として、これを許可しないことを強く推奨しています。

この更新プログラムはどのように問題を修正しますか?
この更新プログラムは Microsoft Office がファイルを開く方法を変更することにより、この脆弱性を解決します。
...
<td style="border:1px solid black;">
officexp-KB977896-FullFile-JPN /q:a
</td>
</tr>
<tr>
<td style="border:1px solid black;">
...
<td style="border:1px solid black;">
officexp-KB977896-FullFile-JPN /r:n
</td>
</tr>
<tr>
<td style="border:1px solid black;">
...
|------------|-------------|------------|-------|-----------|
| Ietag.dll  | 10.0.6731.0 | 2009/12/01 | 15:14 | 105,152   |
| Mso.dll    | 10.0.6858.0 | 2009/12/01 | 15:10 | 9,811,792 |
...
[Microsoft Mactopia Web サイト](https://www.microsoft.com/japan/mac/downloads.mspx)から Microsoft Office 2004 for Mac 11.5.7 用の更新プログラムの適切な言語バージョンをダウンロードし、インストールします。
...
バージョン番号が11.5.7 であれば、この更新プログラムは正常にインストールされています。
...
-   「MSO.DLL のバッファー オーバーフロー」 - [CVE-2010-0243](https://cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2010-0243) の 問題を報告してくださった [Core Security Technologies](https://www.coresecurity.com/) の Damian Frizza 氏
