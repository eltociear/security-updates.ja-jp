---
TOCTitle: 'MS07-019'
Title: 'マイクロソフト セキュリティ情報 MS07-019 - 緊急'
ms:assetid: 'ms07-019'
ms:contentKeyID: 61228969
ms:mtpsurl: 'https://technet.microsoft.com/ja-JP/library/ms07-019(v=Security.10)'
---

マイクロソフト セキュリティ情報 MS07-019 - 緊急
===============================================

ユニバーサル プラグ アンド プレイの脆弱性により、リモートでコードが実行される (931261)
--------------------------------------------------------------------------------------

公開日: 2007年4月11日 | 最終更新日: 2007年4月11日

[](https://www.microsoft.com/japan/security/bulletins/ms07-019e.mspx)
...
**推奨する対応策** **:** 客様はこの更新プログラムを直ちにインストールしてください。
...
|                                                                                                                   | PC/AT                                                                                                                                                                                             | MU                                                                            |
|-------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------|
| Microsoft Windows XP Service Pack 2                                                                               | [![](../../images/Dn610143.dl_arrow(ja-JP,Security.10).jpg)](https://www.microsoft.com/download/details.aspx?familyid=ecf69778-91f9-498e-a8bd-35208aa93051&displaylang=ja) | ![](../../images/Dn610143.mu_s(ja-JP,Security.10).gif) |
| Microsoft Windows XP Professional x64 Edition および Microsoft Windows XP Professional x64 Edition Service Pack 2 | [![](../../images/Dn610143.dl_arrow(ja-JP,Security.10).jpg)](https://www.microsoft.com/download/details.aspx?familyid=6ceb5b4f-861f-4f37-b4bc-e8a56382b833&displaylang=ja) | ![](../../images/Dn610143.mu_s(ja-JP,Security.10).gif) |

![](../../images/Dn610143.dl_arrow(ja-JP,Security.10).jpg)このマークをクリックして、更新プログラムをダウンロードしてください。
更新プログラムをダウンロードおよびインストールする方法は、[更新プログラムのダウンロード方法](https://www.microsoft.com/japan/security/bulletins/dcsteps_vista.mspx)および[更新プログラムのインストール方法](https://www.microsoft.com/japan/security/bulletins/instsecupdate_vista.mspx)をご覧ください。

![](../../images/Dn610143.mu_s(ja-JP,Security.10).gif)このマークの付いている更新プログラムは [Microsoft Update](https://update.microsoft.com/microsoftupdate/) からインストールすることもできます。
...
| 脆弱性識別番号                                                                                              | 脆弱性の影響                 | Microsoft Windows XP Service Pack 2 |
|-------------------------------------------------------------------------------------------------------------|------------------------------|-------------------------------------|
| UPnP メモリの破損の脆弱性 -[CVE-2007-1204](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2007-1204) | リモートでコードが実行される | 緊急                                |
...
Universal Plug and Play のサービスの特別に作成された HTTP のリクエストを処理する方法に、リモートでコードが実行される脆弱性が存在します。攻撃者がこの脆弱性を悪用した場合、ローカル サービスのコンテキストで任意のコードが実行される可能性があります。

#### 「UPnP メモリの破損の脆弱性」の問題を緩和する要素 - CVE-2007-1204:

-   攻撃者がこの脆弱性を悪用した場合、ローカル サービスのアカウントのコンテキストでのみコードが実行され、ローカル システムのアカウントでは、コードが実行される可能性はありません。
-   UPnP のサービスは影響を受けるコンピュータで、既定で手動に設定されています。
...
#### 「UPnP メモリの破損の脆弱性」の回避策 - CVE-2007-1204:
...
-   **ファイアウォールにより、以下のポートをブロックする**
    ...
    この UPnP のフレームワークでは、UDP ポート 1900 および TCP ポート 2869 を使用しています。Simple Service Discovery Protocol (SSDP) は、マルチキャスト検索を使用して UPnP のデバイスを検出します。マルチキャスト検索は動的な送信ポートを使用して UDP ポート 1900 へ送信されます。Windows ファイアウォールは、マルチキャスト検索の実行から 3 秒以内に受信した、検索要求と一致する応答を受け入れます。その後、ファイアウォールは検索の要求と一致していたとしてもマルチキャスト検索の応答をブロックします。ファイアウォールを設定した場合の、UPnP サービスへの影響に関する追加情報は、サポート技術情報 [886257](https://support.microsoft.com/kb/886257)をご覧ください。

    **回避策の影響** **:**

    -   UPnP のフレームワークはネットワークに接続している UPnP のデバイスがアナウンスしていても、それを検出できません。このような着信アナウンスはファイア ウォールによってブロックされます。
    -   別のコンピュータ上で実行されているコントロール ポイントは、Windows XP SP2 を実行しているコンピュータ上で動作している UPnP デバイスの検出または制御をできません。このファイアウォールは UPnP デバイス関連の着信メッセージをブロックします。

    **回避策の解除方法** **:**

    回避策の解除には、UDP ポート 1900 および TCP ポート 2869 をファイアウォール上で再び有効にする必要があります。

-   **Universal Plug and Play** **サービスを無効にする**

    UPnP サービスを無効にすることにより、影響を受けるコンピュータを脆弱性の悪用から守る手助けとなります。UPnP サービスを無効にするためには、次のステップを行ってください。

    1.  \[スタート\] をクリックして \[コントロール パネル\] をクリックします。もう 1 つの方法としては、\[設定\] をポイントし、 \[コントロール パネル\] をクリックします。
    2.  \[管理ツール\] をダブルクリックします。
    3.  \[サービス\] をダブルクリックします。
    4.  \[Universal Plug and Play Device Host\] をダブルクリックします。
    5.  \[スタートアップの種類\] の一覧で、\[無効\] をクリックします。
    6.  \[停止\] をクリックして、\[OK\] をクリックします。

    次のコマンドをコマンド プロンプトで実行することにより、UPnP サービスを停止および無効にすることもできます。

    `sc stop UPnPHost &sc config UPnPHost start= disabled`

    **回避策の影響** **:**UPnP サービスを無効にした場合、UPnP サービスに依存しているコンポーネントが正しく動作しなくなる場合があります。

    **回避策の解除方法** **:**

    この回避策を解除するには、UPnP サービスを再び有効にします。これを行うためには、次のステップに従ってください。
...
#### 「UPnP メモリの破損の脆弱性」のよく寄せられる質問 - CVE-2007-1204:

**どのようなことが起こる可能性がありますか?**

Universal Plug and Play (UPnP) のサービスの、特別に作成された HTTP のリクエストを処理する方法に、リモートでコードが実行される脆弱性が存在します。 これらの HTTP のリクエストが、同じサブネットの攻撃者から標的となるコンピュータだけに直接送信される可能性があります。 Windows XP のファイアウォールおよびプロトコルは、このサブネットの制限を強化します。 攻撃者がこの脆弱性を悪用した場合、ローカル サービスのアカウントのコンテキストで任意のコードが実行される可能性があります。

**何が原因で起こりますか?**

UPnP サービスが特別な細工がされた HTTP リクエストを処理する際に、攻撃者が任意のコードを実行するような方法で、システム メモリを破損する可能性があります。

**UPnP** **とは何ですか？**

UPnP は、コンピュータ ネットワーク プロトコルのセットで、家庭用と業務用のインテリジェント デバイスのネットワーキングを簡素化するためにプラグ アンド プレイを拡張します。UPnP の技術が組み込まれたデバイスが物理的にネットワークに接続された場合に、自動的にネットワーク上でお互いに接続し、ユーザー設定または中央のサーバーは必要ありません。
...
ワークステーションおよびクライアント コンピュータが、主にこの脆弱性による危険にさらされます。サーバーおよびターミナル サーバーは影響を受ける UPnP コンポーネントを含まないため、危険にさらされません。UPnP の機能が使用されているコンポーネントがインストールされているコンピュータの場合、UPnP のサービスによりコンピュータが危険にさらされる可能性があります。

**この脆弱性がインターネットで悪用される可能性はありますか?**

標的にされたコンピュータと同じサブネット上の攻撃者によりこの脆弱性が悪用される可能性があります。 これらの HTTP のリクエストが、同じサブネットの攻撃者から標的となるコンピュータだけに直接送信される可能性があります。 また、境界領域のファイアウォールの最善策および標準の境界領域のファイアウォールの構成により、インターネット経由での攻撃からコンピュータを保護することができます。

**この更新プログラムはどのように問題を修正しますか?**

この更新プログラムは、データが割り当てられたバッファに送られる前に UPnP による HTTP の要求の検証方法を変更することにより脆弱性を削除します。
...
このセキュリティ更新プログラムを適用するには、Microsoft Windows XP Service Pack 2 または Windows XP Professional, x64 Edition がインストールされている必要があります。Windows XP の最新のサービス パックの入手方法に関する詳細情報は、マイクロソフト サポート技術情報 [322389](https://support.microsoft.com/kb/322389) をご覧ください。Windows XP Professional, x64 Edition 用の Service Pack 2 の入手方法の詳細は、 [ダウンロード センター](https://www.microsoft.com/download/details.aspx?displaylang=ja&familyid=893fd6c0-6283-44c3-bb84-b2f0315b2ae6)をご覧ください。
...
**WindowsXP-KB931261-x86-JPN /quiet**

**注: /quiet** スイッチを使用すると、すべてのメッセージが表示されなくなります。これは、エラー メッセージを表示しなくなることも含みます。管理者は **/quiet** スイッチを使用する場合、インストールが正常に完了したことを確認するためのサポートされている方法の 1 つを使用してください。また、管理者はこのスイッチを使用する場合、エラー メッセージについて KB931261.log ファイルを確認してください。

コンピュータを強制的に再起動せずにセキュリティ更新プログラムをインストールするためには、Windows XP のコマンド プロンプトで次のコマンドを使用してください。

**WindowsXP-KB931261-x86-JPN /norestart**
...
システム管理者は Spuninst.exe ユーティリティを使用し、このセキュリティ更新プログラムを削除することができます。Spuninst.exe ユーティリティは %Windir%\\$NTUninstallKB931261$\\Spuninst フォルダにあります。このユーティリティは次のセットアップ スイッチをサポートします。
...
|--------------|---------------|----------|------|---------|----------|
| upnphost.dll | 5.1.2600.3077 | 2007/2/6 | 5:18 | 183,296 | SP2GDR   |
| upnphost.dll | 5.1.2600.3077 | 2007/2/6 | 5:20 | 183,296 | SP2QFE   |

Windows XP Professional x64 および Windows XP Professional x64 Edition Service Pack 2:

| ファイル名    | バージョン    | 日付     | 時間 | サイズ  | CPU | フォルダ    |
|---------------|---------------|----------|------|---------|-----|-------------|
| upnphost.dll  | 5.2.3790.2878 | 2007/2/8 | 8:41 | 320,512 | x64 | SP1GDR      |
| wupnphost.dll | 5.2.3790.2878 | 2007/2/8 | 8:41 | 188,928 | x86 | SP1GDR\\wow |
| upnphost.dll  | 5.2.3790.2878 | 2007/2/8 | 8:41 | 320,512 | x64 | SP1QFE      |
| wupnphost.dll | 5.2.3790.2878 | 2007/2/8 | 8:41 | 188,928 | x86 | SP1QFE\\wow |
| upnphost.dll  | 5.2.3790.4019 | 2007/2/8 | 8:52 | 320,512 | x64 | SP2GDR      |
| wupnphost.dll | 5.2.3790.4019 | 2007/2/8 | 8:52 | 188,928 | x86 | SP2GDR\\wow |
| upnphost.dll  | 5.2.3790.4019 | 2007/2/8 | 8:40 | 320,512 | x64 | SP2QFE      |
| wupnphost.dll | 5.2.3790.4019 | 2007/2/8 | 8:40 | 188,928 | x86 | SP2QFE\\wow |
...
-   **レジストリキーの確認**
    ...
    **注:** これらのレジストリ キーには、インストールされるファイルの完全な一覧が含まれない場合があります。また、管理者あるいは OEM メーカーによって 931261 のセキュリティ更新プログラムが Windows インストール ソース ファイルに統合またはスリップストリーム化されている場合、このレジストリ キーが適切に作成されない場合があります。
...
-   UPnP メモリの破損 の脆弱性 - [CVE-2007-1204](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2007-1204) を報告してくださった [IDefense Labs](https://labs.idefense.com/) の Greg MacManus 氏
...
-   US マイクロソフトセキュリティ情報（MS07-019）
    [https://www.microsoft.com/technet/security/bulletin/MS07-019.mspx](https://technet.microsoft.com/security/bulletin/ms07-019)
-   サポート技術情報 （KB） 文書番号 ：931261
    [\[MS07-019\] ユニバーサル プラグ アンド プレイの脆弱性により、リモートでコードが実行される](https://support.microsoft.com/kb/931261)
