---
TOCTitle: 'MS12-042'
Title: 'マイクロソフト セキュリティ情報 MS12-042 - 重要'
ms:assetid: 'ms12-042'
ms:contentKeyID: 61229419
ms:mtpsurl: 'https://technet.microsoft.com/ja-JP/library/ms12-042(v=Security.10)'
---
...
このセキュリティ更新プログラムは Microsoft Windows に存在する 1 件の一般に公開された脆弱性および 1 件の非公開で報告された脆弱性を解決します。これらの脆弱性により、攻撃者が影響を受けるシステムにログオンし、この脆弱性を悪用する特別に細工されたアプリケーションを実行した場合、特権が昇格される可能性があります。この脆弱性が悪用されるには、有効な資格情報を所有し、ローカルでログオンできることが攻撃者にとっての必要条件となります。リモートで、または匿名ユーザーが、この脆弱性を悪用することはないと思われます。

このセキュリティ更新プログラムの深刻度はすべての 32-bit エディションの Windows XP および Windows Server 2003、Windows 7 for x64-based Systems、および Windows Server 2008 R2 for x64-based Systems について「重要」と評価されています。詳細情報については、このセクションの「影響を受けるソフトウェアおよび影響を受けないソフトウェア」のサブセクションを参照してください。

この更新プログラムは Windows ユーザー モード スケジューラーが特定のシステム要求を処理する方法および Windows が BIOS ROM を管理する方法を修正することにより、この脆弱性を解決します。これらの脆弱性の詳細については、次の「脆弱性の情報」のセクションの特定の脆弱性に関するサブセクション「よく寄せられる質問 (FAQ)」を参照してください。
...
<td style="border:1px solid black;">
[MS11-098](https://go.microsoft.com/fwlink/?linkid=232424) の KB2633171 は KB2707511 に置き換えられる
</td>
</tr>
...
<td style="border:1px solid black;">
[MS11-068](https://go.microsoft.com/fwlink/?linkid=223578) の KB2556532 は KB2709715 に置き換えられる
</td>
</tr>
<tr class="alternateRow">
<td style="border:1px solid black;">
...
<th style="border:1px solid black;" >
ユーザー モード スケジューラーのメモリ破損の脆弱性 - CVE-2012-0217
</th>
<th style="border:1px solid black;" >
BIOS ROM 破損の脆弱性 - CVE-2012-1515
</th>
<th style="border:1px solid black;" >
総合的な深刻度
</th>
...
Windows ユーザー モード スケジューラーがシステム要求を処理する方法に特権の昇格の脆弱性が存在します。攻撃者によりこの脆弱性が悪用された場合、カーネルモード内の任意のコードが実行される可能性があります。その後、攻撃者はプログラムのインストール、データの表示、変更、削除、または完全な管理者権限を持つ新たなアカウントを作成する可能性があります

Common Vulnerabilities and Exposures のリストの標準のエントリとしてこの脆弱性を確認するには、[CVE-2012-0217](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2012-0217) を参照してください。

#### ユーザー モード スケジューラーのメモリ破損の脆弱性の問題を緩和する要素 - CVE-2012-0217

緩和する要素は、既定の状態における設定、一般的な構成または最善策を示し、脆弱性悪用の深刻度が下がる場合があります。お客様の状況で、次の「緩和する要素」が役立つ場合があります。

-   この脆弱性が悪用されるには、有効な資格情報を所有し、ローカルでログオンできることが攻撃者にとっての必要条件となります。リモートで、または匿名ユーザーが、この脆弱性を悪用することはないと思われます。
-   この脆弱性は、Intel x64 ベース バージョンの Windows 7 および Windows Server 2008 R2 のみに影響を及ぼします。
-   AMD または ARM ベースの CPU を使用しているシステムはこの脆弱性の影響を受けません。

#### 「ユーザー モード スケジューラーのメモリ破損の脆弱性」の回避策 - CVE-2012-0217

マイクロソフトは、この脆弱性の回避策を確認していません。

#### 「ユーザー モード スケジューラーのメモリ破損の脆弱性」のよく寄せられる質問 - CVE-2012-0217

この脆弱性により、どのようなことが起こる可能性がありますか?   
これは、特権の昇格の脆弱性です。

何が原因で起こりますか?   
この脆弱性は、Windows ユーザー モード スケジューラーが特定のシステム要求を正しく処理しないことにより起こります。

ユーザー モード スケジューラー (UMS) とは何ですか ?   
ユーザー モード スケジューラー (UMS) は、アプリケーションが自らのスレッドをスケジュールするために使用できるシステム API を備えた軽量なメカニズムです。UMS スレッドがカーネルでブロックされた場合、アプリケーションはシステム スケジューラーの介在なしにユーザー モードで UMS スレッドを切り替えて、プロセッサの制御を取り戻すことができます。
...
この更新プログラムは Windows ユーザー モード スケジューラーが特定のシステム要求を処理する方法を修正することにより、この脆弱性を解決します。
...
Windows が BIOS メモリを処理する方法に特権の昇格の脆弱性が存在します。攻撃者によりこの脆弱性が悪用された場合、カーネルモード内の任意のコードが実行される可能性があります。その後、攻撃者はプログラムのインストール、データの表示、変更、削除、または完全な管理者権限を持つ新たなアカウントを作成する可能性があります

Common Vulnerabilities and Exposures のリストの標準のエントリとしてこの脆弱性を確認するには、[CVE-2012-1515](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2012-1515) を参照してください。

#### 「BIOS ROM 破損の脆弱性」の問題を緩和する要素 - CVE-2012-1515

緩和する要素は、既定の状態における設定、一般的な構成または最善策を示し、脆弱性悪用の深刻度が下がる場合があります。お客様の状況で、次の「緩和する要素」が役立つ場合があります。

-   この脆弱性が悪用されるには、有効な資格情報を所有し、ローカルでログオンできることが攻撃者にとっての必要条件となります。リモートで、または匿名ユーザーが、この脆弱性を悪用することはないと思われます。

#### 「BIOS ROM 破損の脆弱性」の回避策 - CVE-2012-1515

マイクロソフトは、この脆弱性の回避策を確認していません。

#### 「BIOS ROM の脆弱性」のよく寄せられる質問 - CVE-2012-1515

この脆弱性により、どのようなことが起こる可能性がありますか?   
これは、特権の昇格の脆弱性です。

何が原因で起こりますか?   
この脆弱性は Windows が BIOS ROM を正しく保護しないことが原因で起こります。
...
この更新プログラムはどのように問題を修正しますか?   
この更新プログラムは、Windows が BIOS ROM を管理する方法を修正して、この脆弱性を解決します。

このセキュリティ情報の公開時に、この脆弱性は一般に知られていましたか?   
はい。この脆弱性は一般で公開されていました。これは Common Vulnerability and Exposure の番号 [CVE-2012-1515](https://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2012-1515) が割り当てられています。
...
WindowsXP-KB2707511-x86-jpn.exe /quiet</td>
</tr>
<tr class="even">
<td style="border:1px solid black;">再起動しないでインストールする</td>
<td style="border:1px solid black;">Windows XP Service Pack 3:<br />
WindowsXP-KB2707511-x86-jpn.exe /norestart</td>
</tr>
<tr class="odd">
...
<td style="border:1px solid black;">[コントロール パネル] の [プログラムの追加と削除] または %Windir%\$NTUninstallKB2707511$\Spuninst フォルダーの Spuninst.exe ユーティリティを使用します。</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;">ファイルに関する情報:</td>
<td style="border:1px solid black;"><a href="https://support.microsoft.com/kb/2707511/ja">マイクロソフト サポート技術情報 2707511</a> を参照してください。</td>
</tr>
<tr class="even">
...
WindowsServer2003-KB2707511-x86-jpn.exe /quiet</td>
</tr>
<tr class="even">
<td style="border:1px solid black;">再起動しないでインストールする</td>
<td style="border:1px solid black;">すべてのサポートされている 32 ビット版の Windows Server 2003:<br />
WindowsServer2003-KB2707511-x86-jpn.exe /norestart</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;">ログ ファイル</td>
<td style="border:1px solid black;">KB2707511.log</td>
...
<tr class="even">
<td style="border:1px solid black;">ファイルに関する情報:</td>
<td style="border:1px solid black;"><a href="https://support.microsoft.com/kb/2709715/ja">マイクロソフト サポート技術情報 2709715</a> を参照してください。</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;">レジストリ キーの確認</td>
...
-   ユーザー モード スケジューラーのメモリ破損の脆弱性 (CVE-2012-0217) を報告してくださった [Bromium](https://www.bromium.com/) の Rafal Wojtczuk 氏および [SUSE](https://www.suse.com/) の Jan Beulich 氏
