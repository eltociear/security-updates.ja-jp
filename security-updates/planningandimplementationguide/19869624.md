...
TOCTitle: 運用ガイドの概要
Title: 運用ガイドの概要
ms:assetid: '9519ea6d-b101-4981-b836-1168f32c7f1f'
ms:contentKeyID: 19869624
ms:mtpsurl: 'https://technet.microsoft.com/ja-jp/library/Cc527049(v=TechNet.10)'
---
...
運用ガイドには、ソリューション コンポーネントの長期にわたる保守手順の概略が記載されています。 このガイドでは、Microsoft Solutions for Management (MSM) に基づいて、Microsoft® Windows Server™ 2003 証明書サービスおよび Microsoft インターネット認証サービス (IAS) のコンポーネントを運用、監視、変更、およびサポートするためのタスクと手順をすべて紹介します。 ガイダンスには、管理システムを導入するためのセットアップ タスク、定期的な日別および週別のタスク、システムチェックとスクリプトの監視、バックアップと復元手順、およびトラブルシューティングの手法とツールについて触れます。
