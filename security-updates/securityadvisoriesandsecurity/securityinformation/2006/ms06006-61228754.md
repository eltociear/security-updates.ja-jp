...
**このセキュリティ情報の対象となるユーザー :**  
 マイクロソフト以外のインターネット ブラウザ用の Microsoft Windows Media Player のプラグインを使用しているお客様
...
**警告 :**  
[911564](http://support.microsoft.com/kb/911564)で、この累積的なセキュリティ更新プログラムのインストール時に発生する可能性がある既知の問題に関して説明されています。 また、このサポート技術情報では、これらの問題に対する推奨される解決策に関する説明も記載されています。
...
**注:**  
 MBSA のスキャン結果には、セキュリティ更新プログラムのタイトルが「マイクロソフト以外の インターネット ブラウザ用の Windows Media Player プラグインの脆弱性により、リモートでコードが実行される (911564)」と表示されます。
...
**注:**  
 Inventory Tool for Microsoft Updates をご使用でない場合、SMS のスキャン結果に、セキュリティ更新プログラムのタイトルが「マイクロソフト以外の インターネット ブラウザ用の Windows Media Player プラグインの脆弱性により、リモートでコードが実行される (911564)」と表示されます。
...
-   **Npdsplay.dll ファイルのアクセス制御リストを変更する**
    ...
    **注:**  
 x64 のプラットフォームでは、%programfiles% の代わりに %programfiles(x86)% を使用してください。
  
    **回避策の影響 :**  
 標準ではない EMBED エレメントを使用するマルチメディア コンテンツを再生しようとする Web サイトはマイクロソフト以外のインターネット ブラウザで適切に表示されない可能性があります。 OBJECT エレメントを使用してコンテンツを表示する Web サイトはこの回避策の影響を受けません。
...
-   **レジストリキーの確認**
    ...
    **注:**  
 このレジストリ キーには、インストールされるファイルの完全な一覧が含まれない場合があります。また、管理者または OEM メーカーによって 911564 のセキュリティ更新プログラムを Windows インストール ソース ファイルに統合またはスリップストリーム化されている場合、このレジストリ キーが適切に作成されない場合があります。
