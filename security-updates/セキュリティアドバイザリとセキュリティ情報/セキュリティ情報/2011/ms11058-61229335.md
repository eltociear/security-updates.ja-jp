---
TOCTitle: 'MS11-058'
Title: 'マイクロソフト セキュリティ情報 MS11-058 - 緊急'
ms:assetid: 'ms11-058'
ms:contentKeyID: 61229335
ms:mtpsurl: 'https://technet.microsoft.com/ja-JP/library/ms11-058(v=Security.10)'
---
...
公開日: 2011年8月9日 | 最終更新日: 2011年10月25日
...
このセキュリティ更新プログラムは、非公開で報告された 2 件の Windows DNS サーバーの脆弱性を解決します。これらの脆弱性でより深刻なものが悪用された場合、攻撃者がドメインを登録し、NAPTR DNS リソース レコードを作成したうえで、特別に細工された NAPTR クエリを標的となる DNS サーバーに送信すると、リモートでコードが実行される可能性があります。DNS の役割が有効になっていないサーバーは、危険にさらされません。

このセキュリティ更新プログラムの深刻度は、32-bit および x64-based エディションの Windows Server 2008、および x64-based エディションの Windows Server 2008 R2 で「緊急」です。すべてのサポートされているエディションの Windows Server 2003 では「重要」です。詳細については、このセクションのサブセクション「影響を受けるソフトウェアと影響を受けないソフトウェア」を参照してください。

このセキュリティ更新プログラムでは、DNS サーバーがメモリ内で NAPTR クエリを処理する方法、およびメモリ内のオブジェクトを使用前に初期化する方法を変更することにより、この脆弱性を解決します。これらの脆弱性の詳細については、次の「脆弱性の情報」のセクションの特定の脆弱性に関するサブセクション「よく寄せられる質問 (FAQ)」を参照してください。
...
|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|------------------------------|----------------|---------------------------------------------------------------------|
| [Windows Server 2003 Service Pack 2](http://www.microsoft.com/downloads/details.aspx?familyid=c2d4aa38-9241-465d-8d65-aba73e936d0f)                                                                                | サービス拒否                 | 重要           | [MS09-008](http://technet.microsoft.com/security/bulletin/ms09-008) |
| [Windows Server 2003 x64 Edition Service Pack 2](http://www.microsoft.com/downloads/details.aspx?familyid=2db4098a-f4f9-4211-b55d-5d0df1409c43)                                                                    | サービス拒否                 | 重要           | [MS09-008](http://technet.microsoft.com/security/bulletin/ms09-008) |
| [Windows Server 2003 with SP2 for Itanium-based Systems](http://www.microsoft.com/downloads/details.aspx?familyid=1934acfa-5637-4ae9-9e9a-fc7e58930b7a)                                                            | サービス拒否                 | 重要           | [MS09-008](http://technet.microsoft.com/security/bulletin/ms09-008) |
| [Windows Server 2008 for 32-bit Systems Service Pack 2](http://www.microsoft.com/downloads/details.aspx?familyid=ac2d5122-6f9b-46f5-9840-77aa7ff84308)\*                                                           | リモートでコードが実行される | 緊急           | なし                                                                |
| [Windows Server 2008 for x64-based Systems Service Pack 2](http://www.microsoft.com/downloads/details.aspx?familyid=5f605f6f-3854-403d-878b-637626aef78f)\*                                                        | リモートでコードが実行される | 緊急           | なし                                                                |
| [Windows Server 2008 R2 for x64-based Systems および Windows Server 2008 R2 for x64-based Systems Service Pack 1](http://www.microsoft.com/downloads/details.aspx?familyid=6a86e2cf-4e7d-4012-88c3-6957a3842dd3)\* | リモートでコードが実行される | 緊急           | なし                                                                |
...
<span></span>
なぜこのセキュリティ情報は 2011 年 10 月26 日に更新されたのですか?  
マイクロソフトは、検出ロジックの変更をお知らせし、すべてのサポートされているエディションの Windows Server 2003 について、置き換えられるセキュリティ情報に関する情報を修正するために、このセキュリティ情報を更新しました。セキュリティ更新プログラムのファイルへの変更はありません。システムを正常に更新済みのお客様は、措置を講じる必要はありません。
...
<th style="border:1px solid black;" >DNS NAPTR クエリの脆弱性 - CVE-2011-1966</th>
<th style="border:1px solid black;" >DNS の初期化されていないメモリ破損の脆弱性 - CVE-2011-1970</th>
...
<span></span>
Windows DNS サーバーが特別に細工された NAPTR クエリ文字列をメモリ内で不適切に処理するときに、リモート コード実行の脆弱性が発生します。攻撃者がこの脆弱性を悪用した場合、システムのコンテキストで任意のコードが実行される可能性があります。攻撃者は、その後、プログラムのインストール、データの表示、変更、削除などを行ったり、完全なユーザー権限を持つ新たなアカウントを作成したりする可能性があります。
  
Common Vulnerabilities and Exposures のリストの標準のエントリとしてこの脆弱性を確認するには、[CVE-2011-1966](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2011-1966) を参照してください。
  
#### 「DNS NAPTR クエリの脆弱性」の問題を緩和する要素 - CVE-2011-1966
  
マイクロソフトは、この脆弱性の問題を緩和する要素を確認していません。
  
#### 「DNS NAPTR クエリの脆弱性」の回避策 - CVE-2011-1966
  
回避策は、根本的な脆弱性を正すものではありませんが、更新プログラムを適用するまでの間、既知の攻撃方法の阻止に役立つ設定または構成の変更を示します。マイクロソフトは次の回避策をテストし、回避策が機能性を低下させるかどうかの情報を提供しています。
  
-   DNS サービスを使用しない場合は無効にする
  
    対話的な方法:
  
    1.  \[スタート\] メニューの \[ファイル名を指定して実行\] に "regedit" と入力し、\[OK\] をクリックします。  
    2.  \[HKEY\_LOCAL\_MACHINE\]、\[SYSTEM\]、\[CurrentControlSet\] を順に展開し、\[Services\] を展開します。  
    3.  \[DNS\] をクリックします。  
    4.  \[ファイル\] メニューをクリックして、\[エクスポート\] を選択します。  
    5.  \[レジストリ ファイルのエクスポート\] のダイアログで、「DNS\_configuration\_backup.reg」と入力し、\[保存\] を選択します。  
    6.  \[Start\] という名前の値をダブルクリックし、\[値\] データ フィールドを「4」に変更します。  
    7.  \[OK\] をクリックします。  
    8.  管理者権限で実行しているコマンド プロンプトで、次のコマンドを使用してください。  
        `scstop DNS`
  
    管理された適用スクリプトを使用する
  
    1.  次のコマンドを使用し、管理された展開スクリプトからレジストリ キーをバックアップします。  
        `regedit/e DNS_configuration_backup.reg HKEY_LOCAL_MACHINE\System\CurrentControlSet\Services\DNS`  
    2.  次のテキストを .REG の拡張子でファイルに保存します (例: "Disable\_service\_DNSame.reg")。  
        `Windows Registry Editor Versio``n 5.00`  
        `[HKEY_LOCAL_MACHINE\System\CurrentControlSet\Services\DNS]`  
        `"Start"=dword:00000004`  
    3.  対象のコンピューターで、上記のステップ 2 で作成したレジストリ スクリプトを次のコマンドで実行します。  
    ...
    3.  \[レジストリ ファイルのインポート\] ウィンドウで DNS\_configuration\_backup.reg を選択し、\[開く\] をクリックします。
  
    管理された適用スクリプトを使用する
  
    コマンド プロンプトで次のコマンドを管理者として実行し、レジストリを元の状態に戻します。  
    `Regedit/s DNS_configura``tion_backup.reg`
  
#### 「DNS NAPTR クエリの脆弱性」のよく寄せられる質問 - CVE-2011-1966
  
この脆弱性により、どのようなことが起こる可能性がありますか?   
これはリモートでコードが実行される脆弱性です。
  
何が原因で起こりますか?   
この脆弱性は、権限のない DNS サーバーがメモリ内で特別に細工された NAPTR レコードを不適切に処理し、権限のある DNS サーバーからそのレコードを再帰的に要求したときに発生します。
  
この脆弱性は、どのコンポーネントに影響を及ぼしますか?   
Dns.exe が、この脆弱性の影響を受けるコンポーネントです。
  
この脆弱性により、攻撃者は何を行う可能性がありますか?   
攻撃者がこの脆弱性を悪用した場合、システムのコンテキストで任意のコードが実行される可能性があります。攻撃者は、その後、プログラムのインストール、データの表示、変更、削除などを行ったり、完全なユーザー権限を持つ新たなアカウントを作成したりする可能性があります。
  
攻撃者はこの脆弱性をどのように悪用する可能性がありますか?   
リモートの権限のない攻撃者は、ドメインを登録して NAPTR DNS リソース レコードを作成してから、特別に細工した NAPTR クエリをターゲット DNS サーバーに送信し、この脆弱性を悪用する可能性があります。
  
主にどのコンピューターがこの脆弱性による危険にさらされますか?   
DNS の役割を実行しているサーバーが主にこの脆弱性による危険にさらされます。この役割を有効にしていないサーバーは、危険にさらされません。
  
NAPTR DNS リソース レコードとは何ですか?   
Naming Authority Pointer (NAPTR) DNS リソース レコード (RR) により、DNS を使用して、ドメイン名構文になっていない URI など、さまざまなリソース名のサービスを検索できるようになります。NAPTR の詳細については、[RFC-2915](http://www.ietf.org/rfc/rfc2915.txt) を参照してください。
  
DNS リソース レコードとは何ですか?   
DNS リソース レコード (RR) は、ドメイン内の特定リソースを識別します。詳細については、TechNet の記事 [How DNS Works](http://technet.microsoft.com/ja-jp/library/dd197446(ws.10).aspx) (英語情報) を参照してください。
  
URI とは何ですか?   
Uniform Resource Identifier (URI) とは、インターネットからのリソースおよびネットワーク上のリソースの操作または識別を行うために使用される文字列です。URL は Web サイトなどのリソースを参照する URI の典型的な例です。URI に関する詳細情報は [RFC-2396](http://www.ietf.org/rfc/rfc2915.txt) を参照してください。
  
この更新プログラムはどのように問題を修正しますか?   
この更新プログラムでは、DNS サーバーがメモリ内で NAPTR クエリを処理する方法を変更することにより、この脆弱性を解決します。
...
<span></span>
DNS サーバーが初期化されていないメモリ内のオブジェクトを不適切に処理するときに、サービス拒否の脆弱性が発生します。この脆弱性が攻撃者に悪用された場合、DNS サーバー サービスがターゲット システムで応答を停止する可能性があります。
  
Common Vulnerabilities and Exposures のリストの標準のエントリとしてこの脆弱性を確認するには、[CVE-2011-1970](http://www.cve.mitre.org/cgi-bin/cvename.cgi?name=cve-2011-1970) を参照してください。
  
#### 「DNS の初期化されていないメモリ破損の脆弱性」の問題を緩和する要素 - CVE-2011-1970
  
マイクロソフトは、この脆弱性の問題を緩和する要素を確認していません。
  
#### 「DNS の初期化されていないメモリの破損の脆弱性」の回避策 - CVE-2011-1970
  
マイクロソフトは、この脆弱性の回避策を確認していません。
  
#### 「DNS の初期化されていないメモリの破損の脆弱性」のよく寄せられる質問 - CVE-2011-1970
  
この脆弱性により、どのようなことが起こる可能性がありますか?   
これは、サービス拒否の脆弱性です。
  
何が原因で起こりますか?   
この脆弱性は、DNS サーバーがメモリ内で初期化されていないオブジェクトを不適切に処理し、存在しないドメインのリソース レコードを検索するときに発生します。
  
この脆弱性は、どのコンポーネントに影響を及ぼしますか?   
Dns.exe が、この脆弱性の影響を受けるコンポーネントです。
  
この脆弱性により、攻撃者は何を行う可能性がありますか?   
攻撃者がこの脆弱性を悪用した場合、影響を受けるコンピューターが応答しなくなる可能性があります。
  
攻撃者はこの脆弱性をどのように悪用する可能性がありますか?   
認証されていないリモートの攻撃者は、特別に細工した DNS クエリをターゲット DNS サーバーに送信し、存在しないドメインのリソース レコードを要求することにより、この脆弱性を悪用する可能性があります。
...
この更新プログラムはどのように問題を修正しますか?   
この更新プログラムでは、DNS サーバーがメモリ内でオブジェクトを使用前に初期化する方法を変更することにより、この脆弱性を解決します。
...
<td style="border:1px solid black;">[コントロール パネル] の [プログラムの追加と削除]、または %Windir%\$NTUninstallKB2562485$\Spuninst フォルダーの Spuninst.exe ユーティリティを使用します。</td>
</tr>
<tr class="odd">
<td style="border:1px solid black;">ファイルに関する情報:</td>
<td style="border:1px solid black;"><a href="http://support.microsoft.com/kb/2562485">マイクロソフト サポート技術情報 2562485</a> を参照してください。</td>
</tr>
<tr class="even">
...
-   DNS NAPTR クエリの脆弱性 - CVE-2011-1966 を報告してくださった Grischa Zengel 氏 (Zengel Medizintechnik GmbH)
...
-   2011/09/22 このセキュリティ情報ページを更新し、すべての影響を受けるエディションの Windows Server 2003 について、「影響を受けるソフトウェア」の表の「このプログラムにより置き換えられるセキュリティ情報」の欄から MS11-046 を削除しました。これは検出の変更のみです。セキュリティ更新プログラムのファイルへの変更はありません。システムを正常に更新済みのお客様は、措置を講じる必要はありません。  
-   V1.2 (2011/10/26):影響を受ける一部の構成について、検出ロジックの変更および更新プログラムの置き換えに関するセキュリティ情報の修正を発表しました。セキュリティ更新プログラムのファイルへの変更はありません。詳細については、更新プログラムに関する FAQ を参照してください。
